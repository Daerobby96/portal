...
class SuratKeputusanController extends Controller
{
    ...
    public function index()
    {
        $sks = SuratKeluar::with(['creator', 'jenisSurat'])
            ->whereHas('jenisSurat', function ($query) {
                $query->whereIn('kode', ['SK-YYS', 'SK-PT']);
            })
            ->latest()
            ->paginate(10);

        return view('manajemen-surat::surat-keputusan.index', compact('sks'));
    }

    public function create()
    {
        // Get jenis surat SK
        $jenisSurat = JenisSurat::whereIn('kode', ['SK-YYS', 'SK-PT'])
            ->active()
            ->get();

        return view('manajemen-surat::surat-keputusan.create', compact('jenisSurat'));
    }
    ...
    public function store(Request $request)
    {
        ...
        // Get jenis surat ID based on jenis_sk
        ...
        $sk = SuratKeluar::create([
            ...
            'status' => 'published', // Auto publish untuk SK
            'created_by' => auth()->id(),
            'approved_by' => auth()->id(),
            'approved_at' => now(),
        ]);
        ...
        $pdf = Pdf::loadView($viewPath, ['sk' => $sk]);
    ...
    public function preview(Request $request)
    {
        ...
        // Create a mock SK record without saving to database
        $kodeJenis = $validated['jenis_sk'] === 'yayasan' ? 'SK-YYS' : 'SK-PT';
        $jenisSurat = JenisSurat::where('kode', $kodeJenis)->firstOrFail();
