...
@section('content')
<form action="{{ route('rtm.update', $rTM) }}" method="POST" enctype="multipart/form-data" id="rtm-form">
    @csrf
    @method('PUT')
    <div class="row g-4">
        {{-- Sisi Kiri: Info Umum --}}
        <div class="col-lg-4">
            <div class="card border-0 rounded-2xl bg-white shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] sticky-top" style="top: 80px;">
                <div class="p-4 bg-slate-50/70 border-b border-slate-100 d-flex align-items-center gap-2">
                    ...
                    <h6 class="mb-0 font-bold text-slate-800">Header & Status</h6>
                </div>
                <div class="p-4">
                    <div class="mb-4">
                    ...
                    <div class="mb-4">
                        <label class="text-xs font-bold uppercase tracking-wider text-slate-400 d-block mb-2">Status Rapat</label>
                        <select name="status" class="w-full rounded-xl border border-slate-200 bg-slate-50/50 px-3 py-2.5 text-slate-700 transition-all focus:border-primary focus:bg-white focus:outline-none focus:ring-4 focus:ring-primary/10">
                    ...
                    <div class="mb-4">
                        <label class="text-xs font-bold uppercase tracking-wider text-slate-400 d-block mb-2">File Absensi</label>
                        <div class="p-3 rounded-xl border border-dashed border-slate-200 bg-slate-50/30 d-flex flex-column gap-2">
                            <input type="file" name="file_absensi" class="w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100">
                            @if($rTM->file_absensi)
                                <div class="inline-flex items-center gap-1.5 rounded-lg bg-emerald-50 text-emerald-700 px-2.5 py-1 text-xs font-bold border border-emerald-100">
                                    <i class="bi bi-file-earmark-check-fill fs-6 text-emerald-500"></i>
                                    <span>Sudah Ada: {{ basename($rTM->file_absensi) }}</span>
                                </div>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="d-flex flex-column gap-2">
                        ...
                        <a href="{{ route('rtm.show', $rTM) }}" class="inline-flex items-center justify-center gap-1.5 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-bold text-slate-500 shadow-sm transition-all hover:bg-slate-50 hover:-translate-y-0.5 hover:shadow-md active:translate-y-0">
                            <span>Batal</span>
                        </a>
                    </div>
                </div>
            ...
            <div class="card border-0 rounded-2xl bg-white shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] overflow-hidden">
                ...
                <div class="p-4 bg-gradient-to-b from-white to-slate-50/20">
                    <div class="mb-4 text-section">
                        ...
                        <textarea name="notulensi" id="notulensi" rows="5" class="w-full rounded-xl border border-slate-200 bg-white px-3.5 py-3 text-slate-700 transition-all focus:border-primary focus:outline-none focus:ring-4 focus:ring-primary/10" placeholder="Catatan jalannya rapat...">{{ old('notulensi', $rTM->notulensi) }}</textarea>
                    </div>
                    ...
                    <div class="mb-3 text-section">
                        ...
                        <textarea name="keputusan_manajemen" id="keputusan_manajemen" rows="4" class="w-full rounded-xl border border-slate-200 bg-white px-3.5 py-3 text-slate-700 transition-all focus:border-primary focus:outline-none focus:ring-4 focus:ring-primary/10" placeholder="Resume keputusan strategis lainnya...">{{ old('keputusan_manajemen', $rTM->keputusan_manajemen) }}</textarea>
                    </div>
                </div>
            </div>
        </div>
