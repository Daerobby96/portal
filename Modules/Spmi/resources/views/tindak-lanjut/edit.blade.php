        ...
        <div class="card border-0 rounded-2xl bg-white shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] h-100 overflow-hidden">
            ...
            <div class="p-4">
                <div class="d-flex flex-column gap-3">
                    <div class="d-flex justify-content-between align-items-center py-2 border-b border-slate-100">
                        ...
                        <span class="inline-flex items-center rounded-lg bg-blue-50 px-2 py-0.5 text-xs font-mono font-bold text-blue-600 border border-blue-100">
                            {{ $tindakLanjut->temuan->kode_temuan }}
                        </span>
                    </div>
                    <div class="d-flex justify-content-between align-items-center py-2 border-b border-slate-100">
                        <span class="text-xs font-semibold text-slate-400 uppercase tracking-wider">Kategori</span>
                        <span class="modern-badge-container">{!! $tindakLanjut->temuan->kategori_badge !!}</span>
                    </div>
                    <div class="d-flex justify-content-between align-items-center py-2 border-b border-slate-100">
                        <span class="text-xs font-semibold text-slate-400 uppercase tracking-wider">Kode Audit</span>
                        <span class="text-sm font-semibold text-slate-700">{{ $tindakLanjut->temuan->audit->kode_audit ?? '-' }}</span>
                    </div>
                    <div class="d-flex justify-content-between align-items-center py-2 border-b border-slate-100">
                        <span class="text-xs font-semibold text-slate-400 uppercase tracking-wider">Unit Kerja</span>
                        <span class="text-sm font-semibold text-slate-700 text-end">{{ $tindakLanjut->temuan->audit->unit_yang_diaudit ?? '-' }}</span>
                    </div>
                </div>
                ...
                <div class="mt-4 p-3 rounded-xl border border-blue-100/70 bg-gradient-to-br from-blue-50/40 to-indigo-50/20 relative overflow-hidden">
                    ...
                    <p class="mb-0 text-slate-700 text-sm leading-relaxed font-medium">{{ $tindakLanjut->temuan->uraian_temuan }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="col-lg-8">
        <div class="card border-0 rounded-2xl bg-white shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] overflow-hidden">
            <div class="p-4 bg-slate-50/70 border-b border-slate-100 d-flex align-items-center gap-2">
                ...
                <h6 class="mb-0 font-bold text-slate-800">Edit Tindak Lanjut</h6>
            </div>
            <div class="p-4">
                <form action="{{ route('tindak-lanjut.update', $tindakLanjut) }}" method="POST" enctype="multipart/form-data" class="m-0">
                    @csrf @method('PUT')
                    <div class="row g-4">
                        <div class="col-md-6">
                        ...
                        <div class="col-md-6">
                            <label class="text-xs font-bold uppercase tracking-wider text-slate-400 d-block mb-2">Tanggal Realisasi</label>
                            <input type="date" name="tanggal_realisasi"
                                class="w-full rounded-xl border border-slate-200 bg-slate-50/50 px-3 py-2.5 text-slate-700 transition-all focus:border-primary focus:bg-white focus:outline-none focus:ring-4 focus:ring-primary/10 @error('tanggal_realisasi') is-invalid @enderror"
                                value="{{ old('tanggal_realisasi', $tindakLanjut->tanggal_realisasi?->format('Y-m-d')) }}">
                            @error('tanggal_realisasi') <div class="invalid-feedback mt-1 text-xs">{{ $message }}</div> @enderror
                        </div>
                        ...
                        <div class="col-12">
                            ...
                            <div class="p-3 rounded-xl border border-dashed border-slate-200 bg-slate-50/30 d-flex flex-column gap-2">
                                ...
                                <div class="mb-1">
                                    <a href="{{ asset('storage/' . $tindakLanjut->bukti_tindakan) }}" target="_blank" class="inline-flex items-center gap-1.5 rounded-lg bg-slate-100 text-slate-700 px-3 py-1.5 text-xs font-bold border border-slate-200 transition-colors hover:bg-slate-200">
                                        <i class="bi bi-file-earmark-pdf-fill text-rose-500 fs-6"></i>
                                        <span>Lihat Bukti Saat Ini</span>
                                    </a>
                                </div>
                                @endif
                                ...
                                <div class="text-[10px] font-medium text-slate-400"><i class="bi bi-info-circle me-1"></i>Format: PDF, DOC, DOCX, JPG, PNG (Maksimal: 10MB). Kosongkan jika tidak ingin mengubah.</div>
                            </div>
                        ...
                        <div class="col-12 d-flex gap-2 justify-content-end pt-3 border-t border-slate-100">
                            <a href="{{ route('tindak-lanjut.show', $tindakLanjut) }}" class="inline-flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-bold text-slate-500 shadow-sm transition-all hover:bg-slate-50 hover:-translate-y-0.5 hover:shadow-md active:translate-y-0">
                                <span>Batal</span>
                            </a>
                            <button type="submit" class="inline-flex items-center gap-1.5 rounded-xl bg-primary px-5 py-2.5 text-sm font-bold text-white shadow-sm transition-all hover:bg-primary-dark hover:-translate-y-0.5 hover:shadow-md hover:shadow-primary/20 active:translate-y-0">
                                ...
                                <span>Perbarui Tindak Lanjut</span>
                            </button>
                        </div>
                    </div>
                </form>
