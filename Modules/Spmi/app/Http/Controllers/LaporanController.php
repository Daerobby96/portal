...
class LaporanController extends Controller
{
    public function index()
    {
        ...
        $ringkasan = [
            ...
            'edom' => [
                'total' => \Modules\Spmi\Models\DosenKinerja::when($periode, fn($q) => $q->where('periode_id', $periode->id))->count(),
                'avg' => \Modules\Spmi\Models\DosenKinerja::when($periode, fn($q) => $q->where('periode_id', $periode->id))->avg('total_rerata') ?? 0,
            ],
        ];
    ...
    {
        $periodes = Periode::orderBy('tahun', 'asc')->orderBy('semester', 'asc')->get();
        $indikators = \Modules\Spmi\Models\IndikatorKinerja::orderBy('tipe')->get();

        $tipeFilter = $request->get('tipe', 'IKU');

        // Mengambil data monitoring yang digroup berdasarkan indikator dan periode
        // Sehingga kita bisa memplot nilai/capaian per periode

        $trendData = [];

        $filteredIndikators = $indikators->where('tipe', $tipeFilter);

        foreach ($filteredIndikators as $indikator) {
            $dataPerPeriode = [];
            foreach ($periodes as $periode) {
                // Cari monitoring terakhir untuk indikator ini pada periode ini yang sudah diverifikasi (atau semua jika tidak ada status)
                $monitoring = Monitoring::where('indikator_id', $indikator->id)
        ...
        $labels = $periodes->map(function ($p) {
    ...
    public function exportPdf(Request $request, string $type)
    {
        ...
        switch ($type) {
            ...
            case 'dokumen':
                ...
                $data['ketua_spmi'] = User::role('super_admin')->where('is_active', true)->first();
                ...
                $data['koordinator_dokumen'] = User::role('staff')->where('is_active', true)->first();
            ...
            case 'monitoring':
                ...
                $data['koordinator_monitoring'] = User::role('auditee')->where('is_active', true)->first();
            ...
            case 'edom':
                $periodeId = $request->periode_id ?? Periode::aktif()?->id;
                $data['kinerjas'] = \Modules\Spmi\Models\DosenKinerja::where('periode_id', $periodeId)
            ...
            case 'buku-ami':
                ...
                $data['standars'] = \Modules\Spmi\Models\Standar::with('indikators')->where('is_aktif', true)->get();

                // P: Pelaksanaan
                $data['monitorings'] = Monitoring::with('indikator')
                    ->when($periodeId, fn($q) => $q->where('periode_id', $periodeId))
                    ->get();
                $data['kuesioners'] = \Modules\Spmi\Models\Kuesioner::when($periodeId, fn($q) => $q->where('periode_id', $periodeId))->get();
                ...
                $data['tindakLanjuts'] = \Modules\Spmi\Models\TindakLanjut::with(['temuan.audit', 'penanggungJawab'])
                    ->whereHas('temuan.audit', fn($q) => $q->when($periodeId, fn($q2) => $q2->where('periode_id', $periodeId)))
                    ->get();

                // P: Peningkatan (RTM)
                $data['rtms'] = \Modules\Spmi\Models\RTM::when($periodeId, fn($q) => $q->where('periode_id', $periodeId))->get();
    ...
    public function exportAuditIndividual(Request $request, Audit $audit)
    {
        ...
        $pdf = Pdf::setOptions([
            ...
            'defaultFont' => 'DejaVu Sans',
            'chroot' => [storage_path('app/public'), public_path(), base_path()],
        ])->loadView('spmi::laporan.pdf.audit-individual', $data);
